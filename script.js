(function(){
    ...
    if (s21Modal && openS21ModalBtn && closeS21ModalBtn) {
      openS21ModalBtn.addEventListener('click', () => showModal(s21Modal));
      closeS21ModalBtn.addEventListener('click', () => hideModal(s21Modal));
      s21Modal.addEventListener('click', (e) => { if (e.target === s21Modal) hideModal(s21Modal); });
    }
    ...
    function goToSlide(slideIndex){ if (slideIndex < 0 || slideIndex >= totalSlides) return; currentSlide = slideIndex; updateCarousel(); }
    function startAutoSlide() { if (totalSlides <= 1) return; stopAutoSlide(); autoSlideInterval = setInterval(nextSlide, 5000); }
...
(function(){
    ...
    const preload = (url) => { if (!url) return; const p = new Image(); p.src = url; };
    ...
    function toggleEgg(){
        ...
        if (logoImg.src === newSrc) { showingEgg = !showingEgg; return; }
    ...
    logoWrap.addEventListener('keydown', (e) => { if (e.key === 'Enter') toggleEgg(); });
